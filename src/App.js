...
class App extends Component {
  ...
  inputPercent() {
    ...
    var value = parseFloat(displayValue) / 100;
    this.setState({
      ...
      <div className="calculator">
        ...
        <div className="calculator-keypad">
          ...
          <div className="operator-keys">
            <button className="calculator-key key-divide" onClick={() => this.performOperator('/')}>÷</button>
            <button className="calculator-key key-multiply" onClick={() => this.performOperator('*')}>×</button>
            <button className="calculator-key key-subtract" onClick={() => this.performOperator('-')}>−</button>
            <button className="calculator-key key-add" onClick={() => this.performOperator('+')}>+</button>
            <button className="calculator-key key-equals" onClick={() => this.performOperator('=')}>=</button>
          </div>
        </div>
      </div>
    );
